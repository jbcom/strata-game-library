...
/**
 * Return shape of the plugin's `config` hook. Vite types `config` as a union of
 * hook forms, so the tests model the concrete object this plugin returns rather
 * than casting through `Function`.
 */
type ConfigHook = () => { optimizeDeps: { include: string[] } };
...
describe('strataVitePlugin', () => {
  ...
  describe('plugin factory', () => {
    ...
    it('should return consistent config across instances', () => {
      const a = strataVitePlugin();
      const b = strataVitePlugin();
      const configA = (a.config as unknown as ConfigHook)();
      const configB = (b.config as unknown as ConfigHook)();
