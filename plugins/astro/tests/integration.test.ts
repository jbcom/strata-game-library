...
/**
 * Shape of the `astro:config:setup` hook under test. Astro types this hook with
 * a large context object, but the integration only consumes `updateConfig`, so
 * the tests model exactly that surface rather than casting through `Function`.
 */
type ConfigSetupHook = (params: { updateConfig: (config: unknown) => void }) => void;
...
describe('strataAstro integration', () => {
  ...
  describe('default configuration (no arguments)', () => {
    it('should call updateConfig when viteR3F defaults to true', () => {
      ...
      (integration.hooks['astro:config:setup'] as unknown as ConfigSetupHook)(hookParams);
...
function getNoExternalEntries(): string[] {
  const integration = strataAstro();
  const updateConfig = vi.fn();
  (integration.hooks['astro:config:setup'] as unknown as ConfigSetupHook)(
    createHookParams({ updateConfig })
  );
