...
describe('tokens.css', () => {
  ...
  describe('light mode overrides', () => {
    ...
    it('should override background tokens in light mode', () => {
      const lightModeMatch = content.match(/\[data-theme=['"]light['"]\]\s*\{[^}]*--sl-color-bg:/);
      expect(lightModeMatch).not.toBeNull();
    });
  });
...
describe('starlight.css', () => {
  ...
  describe('hero section', () => {
    ...
    it('should have light mode hero overrides', () => {
      expect(content).toMatch(/:root\[data-theme=['"]light['"]\]\s+\.hero\s+h1/);
    });
  });
  ...
  describe('sidebar styles', () => {
    ...
    it('should highlight current page in sidebar', () => {
      expect(content).toMatch(/aria-current=['"]page['"]/);
    });
  });
...
describe('components.css', () => {
  ...
  describe('badge styles', () => {
    ...
    it('should have light mode badge overrides', () => {
      expect(content).toMatch(/:root\[data-theme='light'\]\s+\.badge-teal/);
      expect(content).toMatch(/:root\[data-theme='light'\]\s+\.badge-amber/);
      expect(content).toMatch(/:root\[data-theme='light'\]\s+\.badge-green/);
    });
  });
  ...
  describe('layer stack', () => {
    ...
    it('should have light mode layer overrides', () => {
      expect(content).toMatch(/:root\[data-theme='light'\]\s+\.layer-0/);
      expect(content).toMatch(/:root\[data-theme='light'\]\s+\.layer-4/);
    });
  });
...
describe('CSS file consistency', () => {
  ...
  it.each(cssFiles)('$name should reference strata design tokens', ({ name, path }) => {
    const content = readFileSync(path, 'utf-8');
