...
vi.mock('tone', () => {
  ...
  const transportStartFn = vi.fn((...args: unknown[]) => {
    mockTransportStart.calls.push(args);
  });
  const transportStopFn = vi.fn((...args: unknown[]) => {
    mockTransportStop.calls.push(args);
  });
  const transportCancelFn = vi.fn((...args: unknown[]) => {
    mockTransportCancel.calls.push(args);
  });

  // The SUT calls `new Tone.Synth(...)`, `new Tone.NoiseSynth(...)`, etc., while
  // the assertions below use `expect(Tone.Synth).toHaveBeenCalled()`. That means
  // each mock must be BOTH a vi.fn() spy AND constructible. Arrow functions have
  // no [[Construct]] slot, so `new` on an arrow-backed mock throws
  // "is not a constructor" — these implementations must stay `function`
  // expressions, hence the biome suppressions.
  return {
    start: vi.fn().mockResolvedValue(undefined),
    now: vi.fn().mockReturnValue(0),
    // biome-ignore lint/complexity/useArrowFunction: must be constructible for `new Tone.Synth()`
    Synth: vi.fn().mockImplementation(function () {
      return makeSynthInstance();
    }),
    // biome-ignore lint/complexity/useArrowFunction: must be constructible for `new Tone.NoiseSynth()`
    NoiseSynth: vi.fn().mockImplementation(function () {
      return makeNoiseSynthInstance();
    }),
    // biome-ignore lint/complexity/useArrowFunction: must be constructible for `new Tone.PolySynth()`
    PolySynth: vi.fn().mockImplementation(function () {
      return makePolySynthInstance();
    }),
    // biome-ignore lint/complexity/useArrowFunction: must be constructible for `new Tone.MonoSynth()`
    MonoSynth: vi.fn().mockImplementation(function () {
      return makeMonoSynthInstance();
    }),
    // biome-ignore lint/complexity/useArrowFunction: must be constructible for `new Tone.Pattern()`
    ...
    // biome-ignore lint/complexity/useArrowFunction: must be constructible for `new Tone.Time()`
    ...
    gainToDb: vi.fn().mockImplementation((gain: number) => {
