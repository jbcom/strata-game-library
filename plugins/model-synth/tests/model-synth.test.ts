...
describe('ModelSynth', () => {
  ...
  describe('character()', () => {
    ...
    // The `rigged: true` workflow is covered in clients.test.ts
    // ("rigs generated characters when requested"), which stubs the rigging
    // sub-API directly. A raw globalThis.fetch stub cannot express that flow:
    // character({ rigged: true }) now performs generate -> refine -> rigging
    // create -> rigging poll, so a fixed-length fetch mock queue is inherently
    // coupled to the request count rather than to observable behaviour.
  });
