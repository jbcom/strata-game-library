...
export default defineConfig({
  // The `./clients/*` subpath is declared in package.json's exports map, so
  // each client module must be emitted as its own entry — otherwise the
  // declared subpath resolves to nothing for a consumer.
  entry: {
    index: 'src/index.ts',
