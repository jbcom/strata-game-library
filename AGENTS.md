...
All logic in `packages/core/` must be pure TypeScript. No React imports, no hooks. This ensures core algorithms are portable and testable. All React Three Fiber components and hooks live in `adapters/r3f/` (`@strata-game-library/r3f`).

### 2. Performance & 3D

- **Object Reuse**: Never create geometries or materials in a render loop. Use `useMemo`.
- **Framerate**: Keep `useFrame` logic minimal.
- **Math**: Use the centralized math utilities in `packages/core/src/core/math/`.
