...
- R3F now exposes `resolveRuntimeCreatureAnimationClipName()`, `playRuntimeCreatureAnimationAction()`, `crossFadeRuntimeCreatureAnimationAction()`, `stopRuntimeCreatureAnimationAction()`, `createRuntimeCreatureAnimationController()`, and `createRuntimeCreatureAnimationStateController()` so loaded Three actions can be controlled through runtime logical animation ids and named states with fade, cross-fade duration/warp, loop, clamp, reset, and time-scale options.
...
- Remaining work is richer animation graphs, guarded transitions, blend trees, and IK beyond R3F clip-track retargeting, R3F logical action/state control, R3F pose application, and loaded R3F/Babylon clip playback; additional physics-engine wrappers/examples beyond the new object-handle and Rapier-handle seams; actual WebP/KTX2 encoder integrations and authoring workflows beyond the new encoder-request export plans; and shader application beyond the current R3F/Babylon material paths.
...
- Runtime creature animations can now be controlled, cross-faded, and grouped into named states through logical clip aliases via `createRuntimeCreatureAnimationController()`, `createRuntimeCreatureAnimationStateController()`, `playRuntimeCreatureAnimationAction()`, `crossFadeRuntimeCreatureAnimationAction()`, and `stopRuntimeCreatureAnimationAction()`.
