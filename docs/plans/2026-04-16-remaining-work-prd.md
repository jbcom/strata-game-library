...
- R3F now exposes `resolveRuntimeCreatureAnimationClipName()`, `playRuntimeCreatureAnimationAction()`, `crossFadeRuntimeCreatureAnimationAction()`, `stopRuntimeCreatureAnimationAction()`, and `createRuntimeCreatureAnimationController()` so loaded Three actions can be controlled through runtime logical animation ids with fade, cross-fade duration/warp, loop, clamp, reset, and time-scale options.
...
- Remaining work is skeletal animation state machines and IK beyond R3F clip-track retargeting, R3F logical action control/cross-fading, R3F pose application, and loaded R3F/Babylon clip playback; additional physics-engine wrappers/examples beyond the new object-handle and Rapier-handle seams; actual WebP/KTX2 encoder integrations and authoring workflows beyond the new encoder-request export plans; and shader application beyond the current R3F/Babylon material paths.
...
- Runtime creature animations can now be controlled and cross-faded through logical clip aliases via `createRuntimeCreatureAnimationController()`, `playRuntimeCreatureAnimationAction()`, `crossFadeRuntimeCreatureAnimationAction()`, and `stopRuntimeCreatureAnimationAction()`.
