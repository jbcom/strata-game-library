...
- Remaining work is richer IK solver constraints beyond core IK planning plus the new R3F target-pose application path and authored showcase, deeper asset-bound animation examples beyond R3F clip-track retargeting, R3F graph-event/logical-action/guarded-state/weighted-blend control, R3F pose application, and loaded R3F/Babylon clip playback; additional physics-engine examples and backend wrappers beyond the new object-handle, Rapier, and Cannon seams; richer procedural-bake authoring workflows beyond the new encoder-request, encoder-execution, and first-party WebP/KTX2 adapter seams; and shader application beyond the current R3F/Babylon material paths.
