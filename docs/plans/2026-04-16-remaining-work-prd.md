...
- R3F now exposes `resolveRuntimeCreatureAnimationClipName()`, `playRuntimeCreatureAnimationAction()`, `stopRuntimeCreatureAnimationAction()`, and `createRuntimeCreatureAnimationController()` so loaded Three actions can be controlled through runtime logical animation ids with fade, loop, clamp, reset, and time-scale options.
...
- Runtime creature animations can now be controlled through logical clip aliases via `createRuntimeCreatureAnimationController()`, `playRuntimeCreatureAnimationAction()`, and `stopRuntimeCreatureAnimationAction()`.
