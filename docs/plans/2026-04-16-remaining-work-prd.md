...
- Remaining work is richer skeletal animation control beyond R3F clip-track retargeting, R3F pose application, and loaded R3F/Babylon clip playback; concrete physics-engine wrappers/examples beyond the new object-handle seam; WebP/KTX2 texture export plus authoring workflows beyond the new PNG bake export; and shader application beyond the current R3F/Babylon material paths.
