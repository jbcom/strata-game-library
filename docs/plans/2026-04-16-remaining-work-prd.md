...
- R3F now exposes `createRuntimeCreaturePoseTargetMap()` and `applyRuntimeCreaturePose()` so runtime poses can be applied to Three rig objects by runtime bone id, logical bone id, or source rig bone name.
...
- Remaining work is authored animation-graph/IK examples and IK solver application paths beyond core graph/IK planning, R3F clip-track retargeting, R3F graph-event/logical-action/guarded-state/weighted-blend control, R3F pose application, and loaded R3F/Babylon clip playback; additional physics-engine examples and backend wrappers beyond the new object-handle, Rapier, and Cannon seams; richer procedural-bake authoring workflows beyond the new encoder-request, encoder-execution, and first-party WebP/KTX2 adapter seams; and shader application beyond the current R3F/Babylon material paths.
