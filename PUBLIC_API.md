...
The R3F adapter consumes those plans through `RuntimeProp`, `RuntimeCreature`, `RuntimeCreatureAsset`, `RuntimeAssetMesh`, and `createRuntimeGeometry()`, with override hooks for custom node/bone renderers and custom Three.js materials. `RuntimeCreatureAsset` exposes `createRuntimeCreatureAssetRigBinding()` and an `onRigBinding` callback so loaded Three.js bone hierarchies can be checked against core creature `boneMap` metadata. `RuntimeProp` can also execute prop interaction actions on node clicks via `onInteraction`, `interactionState`, and `selectInteractionAction`, and `applyRuntimePropInteractionPhysicsEffects()` maps renderer-neutral physics effects to runtime object metadata plus optional physics-adapter callbacks.
