...
`MaterialDefinition.traits`, `createMaterialTrait()`, `inferMaterialTraits()`, `createMaterialProceduralPlan()`, `createMaterialProceduralBakePlan()`, and `rasterizeMaterialProceduralBakePlan()` provide serializable procedural material metadata, deterministic shader/texture layer plans, bake manifests, and pure RGBA bake rasters for grain, fibers, scratches, wear, patina, veins, mottle, and absorption channels. Variants can replace or append traits, adapter material descriptors infer or preserve trait, procedural plan, and procedural bake-plan metadata, the R3F adapter applies those plans to `MeshStandardMaterial` shader compilation, and the Reactylon adapter attaches a Babylon PBR material plugin for procedural albedo, scalar, opacity, and emissive effects.
