...
`CreatureDefinition.assets` and `CreatureComposition.runtime.asset` provide optional model, rig, animation-clip, and bone-map bindings for asset-backed creature rendering. `createCreatureRigBindingPlan()` converts logical creature bones plus optional loaded source rig bone names into deterministic matched/missing/unverified binding metadata. The R3F adapter exposes `RuntimeCreatureAsset`, and `RuntimeCreature` can use asset bindings through `assetMode` and `animation`.
