---
title: Claude entry point
updated: 2026-08-23
status: current
domain: context
---

# CLAUDE.md

> **Claude Code-specific instructions.** For comprehensive project reference, see [AGENTS.md](AGENTS.md).

## Claude-Specific Settings

- Use conventional commits with `Co-Authored-By` trailer
- Run the full quality checklist from AGENTS.md before completing work
- Reference `AGENTS.md` for project architecture and standards
- Read `memory-bank/activeContext.md` at session start for context continuity
- Update `memory-bank/activeContext.md` at session end with work summary

## See Also

- [AGENTS.md](AGENTS.md) -- Primary project reference (architecture, standards, commands)
- [docs/AGENTS.md](docs/AGENTS.md) -- Documentation system
- [memory-bank/AGENTS.md](memory-bank/AGENTS.md) -- Agentic memory
- [.claude/README.md](.claude/README.md) -- Custom slash commands and agents
