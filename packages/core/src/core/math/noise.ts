...
export function warpedNoise2D(
  ...
  const wx =
    x + fbm2DInternal(noise, x + 0.0, y + 0.0, 2, frequency, persistence, lacunarity) * strength;
  const wy =
    y + fbm2DInternal(noise, x + 5.2, y + 1.3, 2, frequency, persistence, lacunarity) * strength;
