...
- `RuntimeProp` and `RuntimeCreature` render resolved core composition runtime plans in R3F, including asset-backed creature rig binding coverage, opt-in Three.js clip-track retargeting, and runtime creature pose application, while `StrataRuntimeProp` and `StrataRuntimeCreature` expose serializable Reactylon/Babylon descriptors for the same plans.
