...
- `RuntimeProp` and `RuntimeCreature` render resolved core composition runtime plans in R3F, while `StrataRuntimeProp` and `StrataRuntimeCreature` expose serializable Reactylon/Babylon descriptors for the same plans, including asset-backed creature rig binding coverage.
